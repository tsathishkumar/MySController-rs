...
impl CommandMessage {
    pub fn new(command_message: &String) -> Result<CommandMessage, String> {
        ...
        let ack = message_parts[3].parse::<u8>().map_err(|_|"ACK is not a number")?;
        let sub_type = message_parts[4].parse::<u8>().map_err(|_|"Sub type is not a number")?;
        let payload = message_parts[5];

        
        Ok(match command {
            CommandType::STREAM => CommandMessage::Stream(stream_message::StreamMessage::build(node_id, child_sensor_id, sub_type, ack, payload)),
...
mod test {
    ...
    fn convert_fw_request_to_response() {
        ...
        let mut command_message = CommandMessage::new(&String::from(message_string)).unwrap();
        command_message.to_response(&Firmware::prepare_fw(
            10,
            2,
            String::from("Blink"),
